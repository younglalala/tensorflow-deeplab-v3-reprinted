...
def atrous_spatial_pyramid_pooling(inputs, output_stride, batch_norm_decay, is_training, depth=256):
  """Atrous Spatial Pyramid Pooling.   #提取高级特征，（语义信息），对应图中encoder部分
  ...
  Returns:
    The atrous spatial pyramid pooling output.无规则空间金字塔汇集输出
  """
  with tf.variable_scope("aspp"):
    if output_stride not in [8, 16]:
      raise ValueError('output_stride must be either 8 or 16.')

    atrous_rates = [6, 12, 18]   #采用不同的卷积膨胀率得到图像的空间的关联性，（3，3）的卷积卷积核膨胀率为（6，12，18）
    if output_stride == 8:
      atrous_rates = [2*rate for rate in atrous_rates]    #12,24,36
    ...
    with tf.contrib.slim.arg_scope(resnet_v2.resnet_arg_scope(batch_norm_decay=batch_norm_decay)):
      with arg_scope([layers.batch_norm], is_training=is_training):
        ...
        # (b) the image-level features   #图像级特征
        with tf.variable_scope("image_level_features"):
          # global average pooling  #指定【1，2】纬度上求平均值，并保证输出的维度和输入的纬度一样，不进行降维
          image_level_features = tf.reduce_mean(inputs, [1, 2], name='global_average_pooling', keepdims=True)
          # 1x1 convolution with 256 filters( and batch normalization)
          image_level_features = layers_lib.conv2d(image_level_features, depth, [1, 1], stride=1, scope='conv_1x1')
          # bilinearly upsample features   #双线性resize把图片缩放成输入图片的大小
...
def deeplab_v3_generator(num_classes,
                         output_stride,
                         base_architecture,   #resnet模型的名称【resnet101,resnet50】
                         pre_trained_model,
                         batch_norm_decay,
  ...
  def model(inputs, is_training):
    ...
    inputs_size = tf.shape(inputs)[1:3]   #
    net = end_points[base_architecture + '/block4']   #取block4的输出作为网络输入
    net = atrous_spatial_pyramid_pooling(net, output_stride, batch_norm_decay, is_training)  #得到高级特征提取部分的信息
    with tf.variable_scope("upsampling_logits"):
      net = layers_lib.conv2d(net, num_classes, [1, 1], activation_fn=None, normalizer_fn=None, scope='conv_1x1')
      logits = tf.image.resize_bilinear(net, inputs_size, name='upsample')   #上采样
...
def deeplabv3_model_fn(features, labels, mode, params):    #要使用Estimator，函数格式要保证features, labels, mode, params，
    #config，返回EstimatorSpec
  ...
  images = tf.cast(
      ...
      tf.uint8)    #mean_image_addition+features的图片
...
#解析label
  ...
  if mode == tf.estimator.ModeKeys.PREDICT:
    ...
    #py_func可以用定义的python函数来运行tensor
  gt_decoded_labels = tf.py_func(preprocessing.decode_labels,
                                 [labels, params['batch_size'], params['num_classes']], tf.uint8)

  labels = tf.squeeze(labels, axis=3)  # reduce the channel dimension.

  logits_by_num_classes = tf.reshape(logits, [-1, params['num_classes']])   #形状变换成（n,21），展平onehot编码
  labels_flat = tf.reshape(labels, [-1, ])  #把label展平

  valid_indices = tf.to_int32(labels_flat <= params['num_classes'] - 1)   #转换成int形式
  #根据索引找值，分成两部分
  ...
  cross_entropy = tf.losses.sparse_softmax_cross_entropy(
      logits=valid_logits, labels=valid_labels)  #计算分类误差
  ...
  # Add weight decay to the loss.加入权重衰减和l2loss
  ...
  if mode == tf.estimator.ModeKeys.TRAIN:
    ...
    train_op = None
#tf.metrics.accuracy（）返回acc，accop
  accuracy = tf.metrics.accuracy(
      valid_labels, valid_preds) #计算准确度
  mean_iou = tf.metrics.mean_iou(valid_labels, valid_preds, params['num_classes'])   #计算iou
